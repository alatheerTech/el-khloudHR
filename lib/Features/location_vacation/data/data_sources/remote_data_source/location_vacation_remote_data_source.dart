...
import 'package:qimam/core/utils/network/network_request.dart';
import 'package:qimam/core/utils/functions/setup_service_locator.dart';
import 'package:qimam/core/utils/network/network_utils.dart';
...
class LocationVacationRemoteDataSourceImpl extends LocationVacationRemoteDataSource {
  @override
  Future<LocationVacationResponse> sendVacation(
    String moderEmpId,
    String empPhoneNum,
    ...
    await getIt<NetworkRequest>()
        .requestFutureData<Map<dynamic, dynamic>>(
      Method.post, // POST method
      params: body,
      url: NewApi.doServerAddLocationVacationApiCall, // endpoint من NewApi
      newBaseUrl: NewApi.baseUrl, // baseUrl من NewApi
      options: Options(contentType: Headers.formUrlEncodedContentType), // form data
      onSuccess: (data) {
  ...
  List<Map<String, dynamic>>? _digForVacationList(dynamic node) {
    if (node is List) {
      final casted = node
          .whereType<Map>()
          .map((item) => Map<String, dynamic>.from(item.map((key, val) => MapEntry(key.toString(), val))))
          .toList();
      if (casted.isNotEmpty && casted.first.containsKey('vacation_id')) {
        return casted;
      }
    } else if (node is Map) {
      final normalized = node.map((key, value) => MapEntry(key.toString(), value));
